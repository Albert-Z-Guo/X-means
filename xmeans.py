...
class XMeans:
    def __init__(self, K_init=1, K_max=20, identical_spherical_normal_distributions=False, split_centroids_mannually=False):
        self.K_init = K_init
        self.K_max = K_max
    ...
    def centroids_subclusters(self, cluster_points, mu, random=False):
        ...
        direction = np.random.random_sample(size=len(mu_n)) if random else (cluster_points - mu)[norms.argmax()]
        vector = direction/np.sqrt(np.dot(direction, direction))*np.quantile(norms, 0.6)
    ...
    def BIC_identical_spherical(self, R_n_list, variance):
        ...
        note that variances are the same across all clusters'''
        K = len(R_n_list)
        R = np.sum(R_n_list)
        if variance == 0: 
    ...
    def log_likelihood(self, R, cluster_points, mu, sigma):
        ...
        return np.sum(np.fromiter((np.log(R_n/R*multivariate_normal.pdf(point, mu, sigma)) for point in cluster_points), np.float16))
    ...
    def BIC(self, cluster_points_list, mu_list):
        ...
        l = 0
        for (cluster_points, mu, sigma) in zip(cluster_points_list, mu_list, sigma_list):
            l += self.log_likelihood(R, cluster_points, mu, sigma)
        return l - self.p(K)/2*np.log(R)
    ...
    def fit(self, X):
        ...
        while(1):
            ...
            model = KMeans(n_clusters=K, init=init, n_init=1).fit(X)
            ...
            for n in range(K):
                ...
                if R_n > 1:
                    ...
                    if self.split_centroids_mannually:
                        ...
                        model_subcluster = KMeans(n_clusters=2, init=centroids_subcluster, n_init=1).fit(points_n)
                    else:
                        model_subcluster = KMeans(n_clusters=2).fit(points_n)
        ...
        model_final = KMeans(n_clusters=K).fit(X)
